// --- Constantes y Utilidades específicas de Palabras Bomba ---
...
function getRoundNumber(turnCount, numPlayers = 2) {
  // En modo por ronda, la letra cambia cada (numPlayers * 2) turnos
  const turnsPerRound = numPlayers * 2;
...
// --- Clase Principal del Juego (Palabras Bomba) ---
class WordGame {
  constructor() {
  ...
  initDOM() {
    ...
    // Navegación Menú Principal - Removida (ahora en app.js)
  ...
  async joinRoom(codeOverride = null) {
    ...
    const guestForm = document.getElementById("lobby-guest-form");
    const guestWait = document.getElementById("lobby-guest-waiting");
    if (guestForm) guestForm.classList.add("hidden");
    if (guestWait) {
      guestWait.classList.remove("hidden");
      guestWait.innerHTML = `<p class="loading-text">Conectando a la sala ${code}<span class="dots">...</span></p>`;
    }
    ...
    if (this.peerReady) {
      try {
        ...
        if (guestWait) guestWait.classList.add("hidden");
        this.joiningRoom = false;
        return;
      }
    }
  ...
  handleNetworkMessage(msg) {
    switch (msg.type) {
      ...
      case "CONNECTED": // Solo Guest
        ...
        document.getElementById("join-error").classList.add("hidden");
        const gf = document.getElementById("lobby-guest-form");
        const gw = document.getElementById("lobby-guest-waiting");
  ...
  startGameSession(state) {
    ...
    if (state.roundsCount) {
      this.roundsCount = state.roundsCount;
    }
  ...
  verifyWord() {
    ...
    if (this.dictionary.size > 0 && !this.dictionary.has(fullWord)) {
