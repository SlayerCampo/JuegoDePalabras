...
class WordGame {
    constructor() {
        ...
        this.myProfile = { name: "Player 1", emoji: EMOJIS[0], lives: 3, isReady: false };
        this.opponentProfile = { name: "Player 2", emoji: EMOJIS[1], lives: 3, isReady: false };
    ...
    initDOM() {
        ...
        document.getElementById('btn-host').addEventListener('click', () => this.setupHost());
        document.getElementById('btn-guest').addEventListener('click', () => this.setupGuest());
        ...
        document.getElementById('btn-verify').addEventListener('click', () => this.verifyWord());

        // Game Over
        document.getElementById('btn-replay').addEventListener('click', () => this.resetGame(true));
    ...
    setupGuest() {
        ...
        this.network.init(); // Inicializa el peer sin ID específico
    }
    ...
    // --- Lógica de Juego ---
    amIActive() {
        return (this.isHost && this.activePlayer === 'host') || (!this.isHost && this.activePlayer === 'guest');
    }
    ...
    updateHeaderUI() {
        ...
        document.getElementById('p1-info').querySelector('.name').innerText = p1.name;
        document.getElementById('p1-info').querySelector('.emoji').innerText = p1.emoji;
        document.getElementById('p1-lives').innerText = '❤️'.repeat(p1.lives) + '🖤'.repeat(3 - p1.lives);

        document.getElementById('p2-info').querySelector('.name').innerText = p2.name;
        document.getElementById('p2-info').querySelector('.emoji').innerText = p2.emoji;
        document.getElementById('p2-lives').innerText = '❤️'.repeat(p2.lives) + '🖤'.repeat(3 - p2.lives);
    }
    ...
    verifyWord() {
        ...
        if (!historyEntry) {
            historyEntry = { round, letter: this.targetLetter, hostWord: '', guestWord: '' };
    ...
    checkGameOverOrContinue(nextState) {
        ...
        if (this.myProfile.lives <= 0 || this.opponentProfile.lives <= 0) {
            const winner = this.myProfile.lives > 0 ? (this.isHost ? 'host' : 'guest') : (this.isHost ? 'guest' : 'host');
    ...
    showGameOver(winner) {
        ...
        const isMe = (winner === 'host' && this.isHost) || (winner === 'guest' && !this.isHost);
